...
func (c *TestCommand) run(args testCommandArgs) (results map[string]*moduletest.Suite, diags tfdiags.Diagnostics) {
	...
	for _, suiteName := range suiteNames {
		ret[suiteName] = &moduletest.Suite{
			Name: suiteName,
		}
	}
...
func (c *TestCommand) View(opts struct{ JUnitXMLFile string }) testCommandView {
...
type testCommandViewHuman struct {
	// For now we need to hold on to the whole Meta object here in
	// practice, because we've not yet unpicked all of the different
	// concerns it holds, but the idea here is to behave as if this
	// type only has access to the input and output streams and
	// the formatting helpers, and not to all of the non-UI things
	// like constructing Terraform contexts, backends, etc.
	meta *Meta
...
func (v *testCommandViewHuman) Results(results map[string]*moduletest.Suite) tfdiags.Diagnostics {
	...
	v.meta.Ui.Output(spew.Sdump(results))
